...
1. run mike to create a new versioned docs site:
   ```bash
   cd docs
   uv run mike deploy <new_version> latest --update-aliases
   ```

2. Commit all changes with a message like "Release vX.Y.Z".
3. Push the gh-pages branch to update the docs site:
   ```bash
   git checkout gh-pages
   git push origin gh-pages
   ```
4. Push the changes to the main branch with all tags:
   ```bash
